...
void dump_queue(struct queue *queue)
{
	...
	printf("Max size %d, size %d%s\n", queue->size - 1,
		   queue->readp <= queue->writep ? queue->writep - queue->readp : queue->size - queue->readp + queue->writep,
		   queue->readp == queue->writep ? " (EMPTY)" : ((queue->writep + 1) % queue->size) == queue->readp ? " (FULL)"
																											: "");
			   ...
			   void *block)
{
	...
	TRACE("read_block: block @0x%llx, %d %s bytes\n", start,
		  SQUASHFS_COMPRESSED_SIZE(c_byte), SQUASHFS_COMPRESSED(c_byte) ? "compressed" : "uncompressed");
...
int read_data_block(long long start, unsigned int size, char *block)
{
	...
	TRACE("read_data_block: block @0x%llx, %d %s bytes\n", start,
		  c_byte, SQUASHFS_COMPRESSED_BLOCK(size) ? "compressed" : "uncompressed");
